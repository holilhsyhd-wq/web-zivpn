...
function normalizeDomain(d) {
  return String(d || "").trim().replace(/^https?:\/\//i, "").replace(/\/+$/g, "");
}
function splitHostPort(domain) {
  ...
  if (host.includes(":")) {
    ...
    const h = host.slice(0, idx);
    if (/^\d+$/.test(p)) { host = h; port = parseInt(p, 10); }
  }
  return { host, port };
}

async function zivpnCreate(domain, auth, password, days) {
  const { host, port } = splitHostPort(domain);
  const url = `http://${host}:${port}${process.env.ZIVPN_CREATE_PATH || "/create/zivpn"}`;
  ...
  if (!data || String(data.status).toLowerCase() !== "success") throw new Error(data?.message || "Create gagal");
...
module.exports = async (req, res) => {
  try {
    ...
    const total = Number(srv.harga || 0) * d;
    if (me.saldo < total) return fail(res, 400, "Saldo tidak cukup");

    const msg = await zivpnCreate(srv.domain, srv.auth, password, d);
    ...
    await adb.runTransaction(async (tx) => {
      ...
      tx.set(adb.collection("transactions").doc(), {
        uid: me.uid, type: "create", amount: total, ref: `create-${Date.now()}`, createdAt: Date.now()
      });
    });
